...
if __name__=='__main__':
    ...
    for i in range(start_epoch, args.epoch):
        for batch, (target_imgs, target_labels, target_domains) in enumerate(target_loader):
            ...
            t_label_pre, _ = DANN(target_imgs, 0)
            
            loss = DANN.ce_loss(t_label_pre, target_labels)
            loss.backward()

            DANN.opt_f.step()
            DANN.opt_l.step()            
            print( " [Batch %d/%d] [loss: %f] " % ( batch+1, len(target_loader), loss.item()), end = '\r' )
