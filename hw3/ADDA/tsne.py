...
class tSNE():
	def __init__(self, args, device):
		...
		self.device = device
		self.ADDA = ADDA(args).to(self.device)
		self.ADDA.load_test_model()
		...
		for batch, (sv_imgs, sv_labels, _) in enumerate(sv_loader):
			...
			if self.tran_di == 'sv_mn':
				feature = self.ADDA.source_feature(sv_imgs)
			if self.tran_di == 'mn_sv':
				feature = self.ADDA.target_feature(sv_imgs)
		...
		for batch, (mn_imgs, mn_labels, _) in enumerate(mn_loader):
			...
			if self.tran_di == 'sv_mn':
				feature = self.ADDA.target_feature(mn_imgs)
			if self.tran_di == 'mn_sv':
				feature = self.ADDA.source_feature(mn_imgs)
			feature = feature.cpu().detach().numpy()
			for i in range(feature.shape[0]):
				self.features.append(feature[i])
