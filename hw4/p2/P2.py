...
class P2():
    ...
    def forward_feat(self, x):
        ...
        x  = self.f_ext(x)
        x = x.view(pseudo_bsize, -1)

        return x

    def forward_classify(self, x, lengths):
        x = self.rnn(x, lengths)   # output_shape = 2 X bsize X hidden_dim
        # x = x.reshape(pseudo_bsize, -1)   # shape = bsize X (2*hidden_dim)
        return self.label_cl(x)

    def forward_rnn_feat(self, x, lengths):
    ...
    def update(self, x, label, lengths):
        self.opt_c.zero_grad()
        self.opt_rnn.zero_grad()

        pred = self.forward_classify(x, lengths)
